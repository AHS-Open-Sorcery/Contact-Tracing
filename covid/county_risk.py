...
class CountyRisk:
    ...
    def __init__(self):
        ...
        self.four_weeks = dt.datetime.strftime(today_d + dt.timedelta(days=28), '%Y-%m-%d')
        ...
        s.rename(columns={"location": "fips"}, inplace=True)
        ...
        self.new_active_cases = dict((fips, s[s.fips == fips].cases[self.four_weeks] - s[s.fips == fips].cases[self.three_weeks]) for fips in self.all_fips)
        ...
        self.ascertainment = asc.Ascertainment(today_d + dt.timedelta(days=28), pd.to_datetime(self.past + dt.timedelta(days=28)))
